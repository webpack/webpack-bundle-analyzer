...
|**`reportFilename`**|`{String}`|Default: `report.html`. Path to the bundle report file that will be generated in `static` mode. You can provide an **absolute path** or a path relative to your webpack output directory (`output.path`).|
...
|**`statsFilename`**|`{String}`|Default: `stats.json`. Name of the webpack stats JSON file that will be generated if `generateStatsFile` is `true`. You can provide an **absolute path** or a path relative to your webpack output directory (`output.path`).|
